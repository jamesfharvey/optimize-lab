...
4. **Deflection** ⚠ — a share of visits (default 2%; conservative default lowered from 10% by decision June 11, 2026) resolved digitally before arrival; reported separately, never blended into "served."
...
**Appointment punctuality (replaces the binary on-time rate).** Lateness = max(0, summon − scheduled), over summoned appointments. Reported baseline vs optimized: `pct_on_time` (lateness ≤ Late OK), `pct_acceptable` (lateness ≤ Late acceptable), `p50_lateness_min`, `p90_lateness_min`, `max_lateness_min`. Percentage denominators are SHOWN appointments, so a shown appointment never summoned counts against both percentages. **Guardrail (redefined June 11, 2026; uniform across presets):** hard requirement on the COMBINED selected lever set only. PASS iff `p90_lateness_optimized ≤ late_acceptable_min` AND (`p90_lateness_optimized ≤ late_ok_min` OR `p90_lateness_optimized ≤ p90_lateness_baseline + 0.5`). The absolute clause asserts the customer-stated promise ("≥90% of appointments start within Late OK"); the relative clause preserves non-degradation for shops whose baseline already runs past Late OK. The 0.5-minute operational tolerance applies ONLY to the relative clause; the absolute clauses use the configured punctuality inputs untouched. Solo lever runs that degrade p90 lateness emit a **guardrail warning** entry in the ResultsReport (a real finding, not a failure — e.g. appointment smoothing alone strains punctuality in appointment-led shops until the other levers relieve capacity).

**Walk-in promise & accuracy (v1.4).** The check-in quote is produced by the engine's own **dispatch-forward forecaster**: it replays the engine's dispatch (same pickers, duration matrix, collision/break/appointment logic) over the live state, **including the expected future arrivals of the active configuration** — post-smoothing appointment share, post-deflection walk-in rate, the configured surge shape and service mix, at deterministic expectation quantiles (no random draws; quotes are stable and seed-independent). Queue-ahead work is discounted by the abandonment curve's conditional survival — the office cannot know who will walk out, but it knows the curve. This models the production forecaster, which knows its own arrival pattern; any future policy change flows into quotes automatically because the forecaster and the simulator share one dispatch model. The quote is **informational only** — it never feeds queueing, routing, or abandonment decisions (asserted by an operational-invariance test).

The promise is communicated as a **range**: center = dispatch-forward quote; band = ± max(2 min, `range_k` × center), `range_k` default 0.15 (⚠ assumption until VE.12.01 + forecast logs allow fitting). Accuracy is scored **against the original check-in range by design** — it anchors visitor planning decisions (stay vs. run errands). `W_accuracy` is asymmetric: 1.0 within the range; beyond the high bound a penalty on (wait − high)/center at `beta_late` (default 0.4); below the low bound a penalty on (low − wait)/center at `beta_early` (default 0.1, ⚠ assumption). The mild early side REPRESENTS the product's update stream — re-forecast pre-summon notifications (VE.02.01.US.09), >x% wait-drop alerts, range communication, and visitor-initiated push-back when summoned early (VE.11.01-adjacent) — through reduced betas rather than simulated message traffic: sim visitors have no leave-and-return behavior, so simulated messages would change zero operational numbers. **Out of scope for v1, listed as v2 candidates pending VE.12.01 ratings + forecast-log data:** revision volatility (told 60, then 40, then 75) and leave-the-lobby behavior.

**Absolute-wait disutility — W_time (v1.5, walk-ins only).** Expectation management softens waiting's cost; it does not erase time actually consumed. Without this term, a perfectly-quoted 43-minute wait scored as full satisfaction (reductio: an accurately-forecast 3-hour wait would have scored 82/100). The missing mechanism:

`W_time = max(time_floor, 1 − delta_wait × max(0, wait − wait_free_min) / wait_ref_min)`

applied multiplicatively alongside the v1.4 terms, which stay exactly as they were. Defaults (all ⚠ assumptions until VE.12.01 ratings allow fitting the wait-vs-satisfaction curve per customer): `wait_free_min` 10 (short waits feel free) · `wait_ref_min` 60 · `delta_wait` 0.25 (an accurately-promised 60-minute wait costs ~21% vs free) · `time_floor` 0.5 (reached near 130 min). Appointment visitors are EXCLUDED — their scheduled time is not "waiting"; the v1.1 lateness curve continues to govern them unchanged. The four constants are restated in every report's assumption flags.

**The three-mechanism wait decomposition (v1.5).** Every CSAT point traces to a named mechanism: (1) **expectation / promise honesty** — `W_wait` plus the asymmetric `W_accuracy` against the communicated range; (2) **absolute time consumed** — `W_time`, new in v1.5; (3) **match quality & duration** — `Base(e,s)` and `W_duration`.

**Findings (v1.4, restated under v1.5).** (a) Honest quoting lifts the *expectation* component of baseline CSAT (~+10 at the time it was measured) — the forecaster is itself a first-order CSAT feature, independent of any optimization lever; with v1.5, absolute-wait penalties now price long baselines truthfully, so the headline reads: **every CSAT point traces to a named mechanism** — expectation honesty, time consumed, or match quality. (b) Matching trades individual quote predictability for aggregate speed: its throughput-leaning routing lets later short visits overtake queued long-service visitors (bounded by the aging cap), which the asymmetric model prices correctly and the product manages through ranges, re-forecast updates and alerts. Runtime note: quoting costs ~10–15× per simulated day — lab-acceptable.
...
| Notes | Reference world; golden vectors; anchor pins 220/day independently | Ticket culture; protected Road Test; deflection 2%, smoothing target 15% (v1.3) | Appointment-led; punctuality binds hardest |
...
- **v1.5 — June 11, 2026.** Absolute-wait disutility (W_time, walk-ins only): `max(time_floor, 1 − delta_wait × max(0, wait − wait_free_min)/wait_ref_min)`, multiplicative alongside the unchanged v1.4 terms. Closes the model gap where a perfectly-quoted long wait scored as full satisfaction. Four new ⚠ csat_model inputs (wait_free_min 10, wait_ref_min 60, delta_wait 0.25, time_floor 0.5), restated in every report's assumption flags; VE.12.01 ratings are the fitting path. Appointments excluded — the v1.1 lateness curve governs them unchanged. Findings restated as the three-mechanism wait decomposition: expectation honesty, time consumed, match quality. Scoring-only change: operational metrics byte-identical.
- **v1.4 — June 11, 2026.** Honest forecaster + promise ranges + asymmetric accuracy (Phases 2.8 + 2.9). The walk-in check-in quote is now the engine's own dispatch-forward forecast (same pickers/durations/collision logic) over the live state WITH the active configuration's expected future arrivals (deterministic expectation, no draws) and abandonment-survival discounting — one source of truth shared by simulator and forecaster; quotes remain informational-only (operational metrics byte-identical, invariance-tested). Promises are ranges (± max(2 min, range_k × center), range_k 0.15 ⚠); W_accuracy is asymmetric (beta_late 0.4 / beta_early 0.1 ⚠), scored against the original check-in range; the product's re-forecast/alert/push-back stream is represented via the reduced early beta, not simulated messages. Findings recorded: honest quoting lifts baseline CSAT ~+10 alone; matching trades individual quote predictability for aggregate speed, priced by the asymmetric model. Quoting costs ~10–15× day-sim runtime (lab-acceptable). legacy beta_accuracy retained as beta_late fallback.
- **v1.3 — June 11, 2026.** Lever-default revision: deflection default lowered 0.10 → 0.02 (schema + University/DMV presets; assumption flag retained); DMV optimized smoothing target 0.50 → 0.15 (baseline booking share stays 0.10). Closes the v1.2 DMV known limit — with the no-show and deflection loss channels shut, optimized in-office served exceeds baseline in both recalibrated presets (University +7.9%, DMV +1.3%). Punctuality guardrail redefined (combined set, all presets): PASS iff p90 ≤ late_acceptable AND (p90 ≤ late_ok OR p90 ≤ baseline p90 + 0.5) — absolute clause asserts the customer promise, relative clause preserves non-degradation where baseline already runs past late_ok; the 0.5-min operational tolerance applies only to the relative clause.
