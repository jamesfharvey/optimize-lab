...
**Findings (v1.4).** (a) Honest quoting lifts baseline CSAT by roughly +10 points on its own — the forecaster is itself a first-order CSAT feature, independent of any optimization lever. (b) Matching trades individual quote predictability for aggregate speed: its throughput-leaning routing lets later short visits overtake queued long-service visitors (bounded by the aging cap), which the asymmetric model now prices correctly and the product manages through ranges, re-forecast updates and alerts. Runtime note: quoting costs ~10–15× per simulated day — lab-acceptable.
